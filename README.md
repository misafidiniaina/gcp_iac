...
A modular Google Cloud foundation for running Kubernetes workloads on GKE Autopilot. This repository demonstrates infrastructure as code, explicit service dependencies, scoped IAM, network access controls, and automated configuration validation.

## Architecture

```mermaid
flowchart TD
    TF[Terraform root] --> API[Project APIs]
    API --> VPC[Custom-mode VPC]
    VPC --> SUB[Regional subnet: 10.0.0.0/24]
    SUB --> GKE[Regional GKE Autopilot cluster]
    API --> SA[Keyless automation service account]
    SA -. Container Viewer role .-> GKE
    IAP[Identity-Aware Proxy] -. SSH to tagged VMs .-> VPC
```

The root configuration creates one cluster, a VPC and subnet, an IAP-only SSH firewall rule, and a read-only automation identity. It enables Compute Engine, IAM, GKE, Resource Manager, Service Usage, and Artifact Registry APIs. Enabling Artifact Registry does **not** create a registry repository.

The `virtual_machine` module is optional and is **not instantiated** by the root. It creates an Ubuntu VM with OS Login and a public IP; it does not install Jenkins or other applications.
